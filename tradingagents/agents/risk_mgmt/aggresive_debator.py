...
Output conversationally as if you are speaking without any special formatting."""
        ...
        start_time = log_llm_start("BEAR_RISK", model_name)
        try:
            response = llm.invoke(prompt)
            log_llm_end("BEAR_RISK", model_name, start_time, response)
        except Exception as e:
            elapsed = time.time() - start_time
            print(f"[LLM - BEAR_RISK] ❌ {model_name} failed after {elapsed:.2f}s: {str(e)}")
            raise
